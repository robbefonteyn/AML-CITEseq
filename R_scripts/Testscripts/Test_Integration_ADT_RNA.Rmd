...
```{r}
p1 <- DimPlot(data, reduction = 'RNA_pca',group.by = "RNA_clusters", label = TRUE, repel = TRUE, label.size = 5) + NoLegend()
p2 <- DimPlot(data, reduction = 'wnn.umap',group.by = "ADT_clusters", label = TRUE, repel = TRUE, label.size = 5) + NoLegend()
