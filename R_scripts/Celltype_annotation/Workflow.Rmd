...
First, you need a Seurat object containing a well annotated reference of cell states encountered in blood and bone marrow. We recommend ours :-)
...
Loading my own data
```{r}
setwd("C:/Users/Robbe Fonteyn/OneDrive - UGent/AJ 2024-2025/Master's_Dissertation/AML-CITEseq/Data/Processed_SCC/PROCESSED/SAB001/SAB001_reseq")
merged <- readRDS("SAB001_reseq_seurat.rds")
```

```{r}
# Assign patient ID to 'Batch'
merged$Batch <- merged$Patient

# Create 'status' column based on patient ID prefix
merged$status <- ifelse(grepl("^HC", merged$Patient), "healthy", "diseased")
```
